...
if df is not None and 'Descripcion' not in df.columns:
    ...
    df['embeddings'] = df['Descripcion'].apply(lambda x: model.encode(x, convert_to_tensor=True))
    ...
    def find_best_matches(description, threshold=0.3, recommendation=False):
        ...
        cosine_similarities = cosine_similarity(user_embedding.unsqueeze(0), list(df['embeddings']))[0]
    ...
    if st.button("Get My Script!"):
        if user_description:
            ...
            if not matches:
                st.write("Sorry, no encontré nada, hora de programar.")
            else:
                ...
                st.write(f"**Me tomó:** {time_taken:.4f} seconds")
        else:
